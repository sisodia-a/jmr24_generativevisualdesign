...
def main(args):
    ...
    # PREPARES DATA
    train_loader, test1_loader, test2_loader, train_loader_unshuffled, train_loader_batch1 = get_dataloaders("watches",batch_size=int(default_config['batch_size']),eval_batchsize=int(default_config['eval_batchsize']),model_name=args.name,sup_signal=args.sup_signal,logger=logger)
    ...
    trainer(train_loader,train_loader_unshuffled,epochs=int(args.epochs),checkpoint_every=int(default_config['checkpoint_every']),)
